...
psfile  = "figs/oat_cedc_crude_event.ps"


st2file = "data/oat_isc_crude.oat2"
psfile  = "figs/oat_isc_crude_event.ps"
...
gmt5.shell("cat %s | awk '{print $3,$2}' > temp.xy" % st2file)
gmt5.shell("sort -u temp.xy > temp2.xy")
gmt5.cmd("psxy", "temp2.xy -J%s -R%s  -K -O -Sc0.1c -W0.35p,red  >> %s" % (Jh, Rh, psfile))
